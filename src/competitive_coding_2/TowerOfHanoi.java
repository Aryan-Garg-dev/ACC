...
public class TowerOfHanoi {
  static void towerOfHanoi(int numDisks, char source, char auxiliary, char destination){
    ...
    for (int i = numDisks; i >= 1; i--) sourceStack.push(i);
    ...
    for (int move = 1; move <= totalMoves; move++){
      if (move % 3 == 1) moveDisk(sourceStack, destinationStack, source, destination);
      else if (move % 3 == 2) moveDisk(sourceStack, auxiliaryStack, source, auxiliary);
      else moveDisk(auxiliaryStack, destinationStack, auxiliary, destination);
    }
  }
